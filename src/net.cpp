...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Thaiclubcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"192.169.197.156", "192.169.197.156"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Thaiclubcoin is probably already running."), addrBind.ToString().c_str());
        else
